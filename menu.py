...
order = []  # An order list is initialized
...
while place_order:
    ...
    if menu_category.isdigit():
        ...
        # 4. Check if the customer's input is a valid option
        if menu_category in menu_items.keys():
            ...
            menu_selection = input("Please input the number corresponding to your menu choice: ")

            # Validate the menu item selection (already validated menu_category)
    ...
    while True:
        ...
        match keep_ordering:
            ...
            case _:
                print("Invalid input. Please type 'Y' or 'N'.")  # Tell the customer to try again
...
for item in order:
    ...
    item_name_formatted = f"{item_name:26}"
    price_formatted = f"${price:5.2f}"
    quantity_formatted = f"{quantity:<8}"

    # 10. Print the item name, price, and quantity
    print(f"{item_name_formatted} | {price_formatted} | {quantity_formatted}")
